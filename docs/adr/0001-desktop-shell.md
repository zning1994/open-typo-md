...
| 代价 | 缓解 |
| --- | --- |
| 安装包大（~100MB+） | 预算 M1.5 放宽到 180MB（120MB 余量太小）。✅ 实测 Linux AppImage 113.6MB；懒加载重型依赖是有效的，KaTeX / Mermaid 都不在主 bundle 里 |
| 内存占用高 | ⚠️ **这条没兑现**。基准做了，实测空闲 550MB 对预算 300MB —— Electron 的进程基座本身就要三四百 MB，预算当初定乐观了。要压下来得动进程结构，见 07 §2.3 |
| 安全面更大（Node 在手边） | 架构 01 §6 的完整安全模型：contextIsolation、sandbox、CSP、路径校验 |
| 冷启动慢 | 预算 1.5s。✅ 实测 807ms，靠的只是渲染层代码分割（主 bundle 1.32MB，懒加载 4.21MB 分 174 个 chunk）；V8 快照还没用上，也还不需要 |
