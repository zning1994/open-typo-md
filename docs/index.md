...
**能用了，还在往 1.0 走。** 版本 0.2.0。M4.5 那一档已经做完：
...
v0.2.0 又加上了：文件树里的新建 / 重命名 / 移到废纸篓、Quick Open（⌘P）、
全工作区搜索（⌘⇧F）、大纲的过滤与折叠、检查更新。

**下一步是 AI**：改写、翻译、总结、问文档，都是内置能力，不需要装任何东西 ——
而且**永远不直接改文档**，只产出一份能看见的 diff，⌘Z 一次就能撤销。
设计见 [10 AI](/design/10-ai)。

**还没有**：AI（M5，一行没写）、用户主题目录与热加载、块级 HTML 渲染
（这条是**决定不做**，理由见路线图）、表格拖拽调列宽、PDF 的页眉页脚与目录页码、
文件树里的拖拽移动、Pandoc 导出（DOCX / ePub）。

**开放插件系统也是决定不做**（v1）：不加载第三方代码。将来要开放的是受控的
Tool API，重估条件见 [ADR-0006](/adr/0006-ai-runtime-and-plugin-strategy)。
