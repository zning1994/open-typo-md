...
值得留下这一段，因为**排查过程比结论有价值**。

症状：产物是一份结构完整的 PDF（页数、纸张、白底矩形都对），但内容流里
**一条绘制文字的指令都没有** —— 一页空白。只在 macOS 上，Linux 与 Windows 正常。
本地跑不出来，只有 CI 上有 macOS 机器。

排查靠的是**把管线拆开**，一轮 CI 分清三种可能：

1. 喂一段写死的 ASCII HTML 给 `renderPdf` → **过了**，隐藏窗口没问题；
2. 换成中文 → **也过了**，不是字体问题；
3. 在测试里手工拼一份「同样的主题 CSS + 同样的排版样式」的文档 → **还是过了**。

于是嫌疑只剩真实产物比手工那份多出来的东西 —— **内联的 KaTeX 样式**。
它带着二十来个 woff2 的 base64，几 MB 起步，而当时**每一次导出都无条件带上它**，
哪怕文档里一个公式都没有。

修法同时也是一条本来就该做的优化：**只在片段里真的出现了 KaTeX 产物时才内联那套
样式**。判据放在片段上而不是原文上 —— `rehypeKatex` 只有真渲染出公式时才会打上
`katex` 类，比在 Markdown 里数 `$` 靠谱得多。

顺带的收益：一篇没有公式的文档，导出产物从几 MB 回到几十 KB。

**留下的教训**：本地只有一个平台，而三个平台的 CI 是唯一的真相。
「本地全绿」在这个项目里不等于「做完了」。
