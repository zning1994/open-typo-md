...
### 3.1 当前实际支持到哪一步（M1.5 实测）

上面那张表说的是「计划支持什么」。这一节说的是「现在实际是什么样」——
两者混在一起，很容易让人以为 CommonMark 已经全须全尾。

**解析层**：CommonMark 全集（652 个官方 spec 用例跑过）。GFM **尚未开启**
（`dialect` 默认 `commonmark`）。

**呈现层**分三类：

**① 有专门的呈现规则**

标题、粗体、斜体、行内代码、引用、无序列表、嵌套列表、分隔线、
行内链接、引用式链接、自动链接、图片。
...
| 围栏代码块 | 等宽 / 底色 / 高亮 / 不折行，``` 可见 | 藏了用户就删不掉这个块 |
| 缩进代码块 | 同上 | 本来就没有可藏的标记 |
...
| 语法 | 现在 | 应该 | 排期 |
| --- | --- | --- | --- |
| 转义字符 `\*` | 显示 `\*` | 藏掉反斜杠 | M2（成本很低） |
| HTML 实体 `&amp;` | 显示 `&amp;` | 显示 `&` | M2 |
| 行内 HTML `<b>x</b>` | 显示标签原文 | 渲染 | M4.5（搁置） |
| HTML 块 | 显示原文 | 渲染 | M4.5（搁置） |
...
- **任务列表看着像「半支持」**：`- [ ] 待办` 会显示成 `• [ ] 待办`。
  那只是无序列表的圆点生效了，`[ ]` 完全没被识别成复选框。
- **删除线的装饰规则已经写在 `build.ts` 里，但它是死代码**：
  方言没开 GFM，`~~x~~` 根本解析不出 `Strikethrough` 节点。M2 翻开关后才生效。
