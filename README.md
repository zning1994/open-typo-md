...
> **状态：M2、M3 完成，M4 进行中**。GFM（表格、任务列表、脚注……）、数学公式、
> 图表、大纲、命令面板、崩溃恢复、5 套主题都已可用；剩下 HTML 导出与
> 复制为富文本 —— 见[路线图](docs/design/08-roadmap.md)。
>
...
- HTML 导出、复制为富文本、用户主题目录（M4，进行中）
- 插件系统（M5）
- 表格网格编辑、文件树与标签页、PDF 导出、设置界面、原始 HTML 渲染
  （**M4.5，明确搁置**，理由与代价见路线图）
