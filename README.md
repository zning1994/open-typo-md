...
- **导出为 PDF**：复用 HTML 导出的产物交给 Chromium 打印，一律浅色。
  ⚠️ **macOS 上目前产出空白页**（Linux / Windows 正常），未定位，见 06 §3.3
- **文件树**（⌘⇧B）：打开一个文件夹，点一下在标签里打开
- **多标签页**（⌘T 新建 / ⌘W 关闭）：撤销栈与脏标记各自独立；
  关窗口时多个未保存文档汇总成一个对话框
- **会话恢复**：下次启动回到上次的工作区与标签
- **设置面板**（⌘,）：主题、新标签默认视图、PDF 的纸张 / 方向 / 页边距

**还没有**（按路线图排期）

- 用户主题目录与热加载（M4 顺延）
- 插件系统（M5）
- 渲染文档里的原始 HTML（**M4.5 仍搁置**，理由与代价见路线图）
- 快捷键自定义、用户主题目录与热加载
