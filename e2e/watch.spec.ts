...
import { docText, expect, openDocInNewWindow, test } from './fixtures.js'
...
async function openDoc(
  ...
  const created = await openDocInNewWindow(app, page, target)
  // 等文档真的装进去了
  await expect.poll(() => docText(created), { timeout: 10_000 }).toContain(content.trim())
  return [created, target]
}

test('外部修改在没有本地改动时静默重载', async ({ app, page }) => {
  // git checkout、外部格式化、云盘同步都走这条路。没有任何东西会丢，就不该问
  const [doc, target] = await openDoc(app, page, 'a.md', '原来的内容\n')
...
test('连改两次都能发现 —— 原子替换换掉 inode 之后 watch 会重新挂', async ({ app, page }) => {
  const [doc, target] = await openDoc(app, page, 'b.md', '第 0 版\n')
...
test('文件被删除时保住缓冲区内容', async ({ app, page }) => {
  // 缓冲区里的内容此刻是唯一一份，自动重载会把它抹掉
  const [doc, target] = await openDoc(app, page, 'd.md', '很重要的内容\n')
